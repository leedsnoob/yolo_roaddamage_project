...
from ultralytics.utils.generate_mask import generate_mask
from ultralytics.utils.loss import region_loss_1mask
...
class BaseTrainer:
    ...
    def _do_train(self):
        ...
        rdd_rg_model = getattr(unwrap_model(self.model), "rdd_rg", False)
        LOGGER.info(
            ...
            for i, batch in pbar:
                ...
                try:
                    with autocast(self.amp):
                        mask_batch = generate_mask(batch) if rdd_rg_model else None
                        batch = self.preprocess_batch(batch)
                        if self.args.compile:
                            # Decouple inference and loss calculations for improved compile performance
                            preds = self.model(batch["img"])
                            loss_output = unwrap_model(self.model).loss(batch, preds)
                        else:
                            loss_output = self.model(batch)

                        if rdd_rg_model:
                            loss, self.loss_items, masks_pred_quarter_logits = loss_output
                            masks_gt = mask_batch.float().to(masks_pred_quarter_logits.device) / 255.0
                            masks_gt_quarter = F.interpolate(
                                masks_gt, size=masks_pred_quarter_logits.shape[-2:], mode="nearest"
                            )
                            loss_mask = region_loss_1mask(masks_pred_quarter_logits, masks_gt_quarter)
                            self.loss = loss.sum() + 3 * masks_pred_quarter_logits.shape[0] * loss_mask
                        else:
                            loss, self.loss_items = loss_output
                            self.loss = loss.sum()
                        if RANK != -1:
