...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, use_wiou: bool = False):
        """Initialize the BboxLoss module with regularization maximum and DFL settings."""
        super().__init__()
        self.dfl_loss = DFLoss(reg_max) if reg_max > 1 else None
        self.use_wiou = use_wiou
        if use_wiou:
            # WIoU dynamic focusing: running mean of IoU, non-monotonic FM params
            self.register_buffer("iou_mean", torch.tensor(1.0))
            self.wiou_momentum = 1e-2
            self.wiou_alpha = 1.7
            self.wiou_delta = 2.7
    ...
    def forward(
        ...
        if self.use_wiou:
            iou, dist_weight = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, WIoU=True)
            # Dynamic non-monotonic focusing mechanism
            if self.training:
                self.iou_mean.mul_(1 - self.wiou_momentum).add_(self.wiou_momentum * iou.detach().mean())
            beta = iou.detach() / self.iou_mean
            focusing = beta / (self.wiou_delta * torch.pow(self.wiou_alpha, beta - self.wiou_delta))
            # WIoU loss = dist_weight * (1 - iou) * focusing
            loss_iou = ((dist_weight * (1.0 - iou) * focusing) * weight).sum() / target_scores_sum
        else:
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        self.bbox_loss = BboxLoss(m.reg_max, use_wiou=getattr(model, "wiou", False)).to(device)
...
def _iou(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    epsilon = 1e-6
    pred = pred.float().clamp(min=epsilon, max=1.0 - epsilon)
    target = target.float()

    intersection = (target * pred).sum(dim=(1, 2, 3))
    union = target.sum(dim=(1, 2, 3)) + pred.sum(dim=(1, 2, 3)) - intersection
...
def region_loss_1mask(pred1: torch.Tensor, mask1: torch.Tensor) -> torch.Tensor:
    mask1 = mask1.float()
    pred1 = pred1.float()
    pred1_prob = pred1.sigmoid().clamp(min=1e-6, max=1.0 - 1e-6)
    loss_bce = F.binary_cross_entropy_with_logits(pred1, mask1)
    loss_iou = IOULoss()(pred1_prob, mask1)
    return loss_bce + 0.6 * loss_iou
