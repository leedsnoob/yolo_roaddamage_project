...
class BaseModel(torch.nn.Module):
    ...
    def predict(self, x, profile=False, visualize=False, augment=False, embed=None):
        ...
        if getattr(self, "rdd_rg", False):
            return self._predict_rdd_rg(x, profile, visualize)
    ...
    def _predict_rdd_rg(self, x, profile=False, visualize=False):
        ...
        inject_idx = cfg["inject"]

        y, dt = [], []
        mask_pred_quarter = torch.zeros((x.shape[0], 1, x.shape[2] // 4, x.shape[3] // 4), device=x.device)
        mask_pred_quarter_logits = mask_pred_quarter
        mask_pred_quarter_attention = None
        input_tensor = x
        ...
        for m in self.model:
            ...
            if m.i < first_pass_end:
                x = m(x)
            if m.i == mask_conv_idx:
                mask_pred_quarter_logits = x
                mask_pred_quarter = torch.sigmoid(x)
                x = mask_pred_quarter
            y.append(x if m.i in self.save else None)
            if m.i == attention_idx:
                mask_pred_quarter_attention = x

        y, dt = [], []
        x = input_tensor
        for m in self.model:
            if m.i == inject_idx and mask_pred_quarter_attention is not None:
                x = mask_pred_quarter_attention + x
            if m.f != -1:
                x = y[m.f] if isinstance(m.f, int) else [x if j == -1 else y[j] for j in m.f]
            if profile:
                self._profile_one_layer(m, x, dt)
        ...
        return x, mask_pred_quarter_logits
    ...
    def loss(self, batch, preds=None):
        ...
        mask_pred_quarter = None
        if getattr(self, "rdd_rg", False):
            preds, mask_pred_quarter = preds
        loss, loss_items = self.criterion(preds, batch)
        if mask_pred_quarter is not None:
            return loss, loss_items, mask_pred_quarter
    ...
    def _resolve_rdd_rg_cfg(self):
        """Resolve and validate RDD-RGNet layer indices from the parsed model graph.

        The original RDD-RGNet repository hard-codes layer numbers in tasks.py.
        This final fork keeps explicit YAML overrides for reproducibility, but
        auto-detects the mask/attention layers when those keys are not present
        ...
        auto_mask = None
        if auto_attention is not None:
            preceding_masks = [idx for idx in mask_indices if idx < auto_attention]
            auto_mask = preceding_masks[-1] if preceding_masks else None
        elif mask_indices:
            auto_mask = mask_indices[0]

        cfg = {
            "mask_conv": self.yaml.get("rdd_rg_mask_conv", auto_mask),
            "attention": self.yaml.get("rdd_rg_attention", auto_attention),
            "first_pass_end": self.yaml.get(
                "rdd_rg_first_pass_end", (auto_attention + 1) if auto_attention is not None else None
            ),
            "inject": self.yaml.get("rdd_rg_inject", 3),
        }

        errors = []
        max_idx = len(self.model) - 1
        for key, value in cfg.items():
            if value is None:
                errors.append(f"{key}=None")
            elif not isinstance(value, int) or value < 0 or value > max_idx:
                errors.append(f"{key}={value} outside model index range 0..{max_idx}")

        if not errors:
            mask_module = self.model[cfg["mask_conv"]]
            attention_module = self.model[cfg["attention"]]
            if not (isinstance(mask_module, nn.Conv2d) and getattr(mask_module, "out_channels", None) == 1):
                errors.append(
                    f"mask_conv={cfg['mask_conv']} is {mask_module.__class__.__name__}, expected nn.Conv2d out_channels=1"
                )
            if attention_module.__class__.__name__ != "binary_spatial_Attention":
                errors.append(
                    f"attention={cfg['attention']} is {attention_module.__class__.__name__}, "
                    "expected binary_spatial_Attention"
                )
            if cfg["first_pass_end"] <= cfg["attention"]:
                errors.append(
                    f"first_pass_end={cfg['first_pass_end']} must be greater than attention={cfg['attention']}"
                )
            if cfg["mask_conv"] >= cfg["attention"]:
                errors.append(f"mask_conv={cfg['mask_conv']} must appear before attention={cfg['attention']}")

        if errors:
            raise ValueError(
                "Invalid RDD-RGNet layer configuration. "
                "Update rdd_rg_mask_conv/rdd_rg_attention/rdd_rg_first_pass_end/rdd_rg_inject in the YAML. "
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo26n.yaml", ch=3, nc=None, verbose=True):
        ...
        self.rdd_rg = bool(self.yaml.get("rdd_rg", False))
        self.wiou = bool(self.yaml.get("wiou", False))
        self.rdd_rg_cfg = {}
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            def _forward(x):
                ...
                if self.rdd_rg and isinstance(output, tuple):
                    output = output[0]
                if self.end2end:
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            args = [ch[f]]
        elif m is binary_spatial_Attention:
            c1 = ch[f]
            c2 = c1
            args = [c1, *args]
        elif m is DySample:
            ...
            if args:
                ...
                c2 = c1
                args = [c1]
        elif m is StripConvAttention:
            c1 = ch[f]
            c2 = make_divisible(min(args[0], max_channels) * width, 8) if args else c1
            args = [c1, c2, *args[1:]]
        elif m is BiFPN_Fuse:
            c2 = make_divisible(min(args[0], max_channels) * width, 8)
            n_inputs = len(f) if isinstance(f, list) else 1
            args = [c2, n_inputs]
        elif m is Concat:
